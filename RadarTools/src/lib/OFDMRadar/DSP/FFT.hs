...
-- | C based fft wrapper
{-# NOINLINE fftV #-}
fftV fftSize direction input = unsafeDupablePerformIO $ do
    
    -- Convert input into a c array
    withArray (inputProper) $ \cinput -> do
        ...
        allocaBytes ((sizeOf (undefined::Double)) * (div fftSize 2)) $ \ccoefs ->
                ...
                allocaBytes ((sizeOf (undefined::Int)) *  ceiling (2.0 + sqrt(fromIntegral fftSize))) $ \cworkarea -> do
                    ...
                    --Do the actual FFT using the c function
                    c_fft cinput ccoefs cworkarea direction fftSize
                    
                    --Get the result
                    list <- peekArray fftSize cinput
                    
                    --Convert back to a vector and return.
                    return $ V.fromList list
                    
                    --Extend the input as needed, though the input should match the size of the FFT.
    where inputProper = if V.length input > fftSize then V.toList $ V.take fftSize input else (V.toList input) ++ (replicate (fftSize - (V.length input)) (0 :+ 0))
...
fft fftSize direction input = unsafeDupablePerformIO $ do
    ...
    where inputProper = input ++ (replicate (fftSize - length input) (0 :+ 0) )
...
createFftV fftSize direction = createFftI fftSize direction (fftV)


-- | Sets up the values to be used by the FFT functions
{-# NOINLINE createFftI #-}
createFftI fftSize direction fftFunc = fftFunc fftSize direction --unsafeDupablePerformIO $ allocaBytes ((sizeOf (undefined::Double)) * (div fftSize 2)) $ \ccoefs -> --Array for Coefs
                    --Note, this "optimization" caused issues with the FFT operation which lead to incorrect results
                    --IP Work area
                    --allocaBytes ((sizeOf (undefined::Int)) *  ceiling (2.0 + sqrt(fromIntegral fftSize))) $ \cworkarea -> do 
                    
                        --C function that sets up the FFT's internal buffers.
                        --c_MakeFft fftSize cworkarea ccoefs
                        
                        -- Get the coefs generated by the fft init function
                        --coefList <- peekArray (div fftSize 2) ccoefs 
                        
                        --Get the IP coefs setup by the fft init function
                        --wkArea <- peekArray (ceiling (2.0 + sqrt(fromIntegral fftSize))) cworkarea 
                        
                        --Return the FFT function 
                        --return $ fftFunc fftSize wkArea coefList direction
...
--Cyclic rotates a matrix by a given shift
